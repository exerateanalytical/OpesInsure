...
final class ProposalController
{
 ...
 /** Mutations: a partner may only act on proposals of clients in their own book. */
 private function bookProposal(string$id):Proposal{$p=$this->proposal($id);app(\App\Application\Partners\PartnerBook::class)->assertInBook(request()->user(),$p->party_id);return $p;}
 ...
 public function store(Request$r,ProposalService$s,TenantContext$c):JsonResponse{$d=$r->validate(['quote_offer_id'=>'required|uuid|exists:quote_offers,id','party_id'=>'required|uuid|exists:parties,id']);$this->own->assertOwnParty($r->user(),$d['party_id']);app(\App\Application\Partners\PartnerBook::class)->assertInBook($r->user(),$d['party_id']);$offer=$this->own->applyVia(QuoteOffer::query(),$r->user(),'quote')->findOrFail($d['quote_offer_id']);return response()->json(['data'=>$s->create(Tenant::findOrFail($c->id()),$offer,$d,$r->user())],201);}
 ...
 public function attachDocument(Request$r,string$proposal,ProposalService$s):JsonResponse{$p=$this->bookProposal($proposal);$d=$r->validate(['document_id'=>'required|uuid|exists:documents,id','requirement_code'=>'required|string|max:64']);return response()->json(['data'=>$s->attachDocument($p,$this->own->apply(Document::query(),$r->user())->findOrFail($d['document_id']),$d['requirement_code'])],201);}
 ...
 public function submit(Request$r,string$proposal,ProposalService$s):JsonResponse{return response()->json(['data'=>$s->submit($this->bookProposal($proposal),$r->user())],202);}
 ...
 public function decide(Request$r,string$case,UnderwritingService$s):JsonResponse{$d=$r->validate(['decision'=>'required|in:APPROVED,CONDITIONAL,COUNTEROFFERED,DECLINED','reason_code'=>'required|string|max:64','notes'=>'required|string|min:20|max:4000','conditions'=>'required_if:decision,CONDITIONAL|array']);return response()->json(['data'=>$s->decide($this->underwritingCase($case),$d,$r->user())]);}
}
