...
final class RiskAssetController
{
 ...
 public function store(Request $r,RiskAssetService $s,TenantContext $c):JsonResponse{$d=$r->validate(['party_id'=>'required|uuid|exists:parties,id','type'=>'required|in:VEHICLE,PROPERTY,TRAVELLER,HEALTH_MEMBER','external_reference'=>'nullable|string|max:100','display_name'=>'required|string|max:160','facts'=>'required|array']);$this->own->assertOwnParty($r->user(),$d['party_id']);return response()->json(['data'=>$s->create(Tenant::findOrFail($c->id()),Party::findOrFail($d['party_id']),$d,$r->user())],201);}
