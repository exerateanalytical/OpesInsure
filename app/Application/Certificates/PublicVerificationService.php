...
final class PublicVerificationService
{
    ...
    public function lookup(string $reference, ?string $token, string $channel, string $fingerprint): array
    {
        ...
        // Short codes are case-insensitive and may be printed grouped (OVXX-XXXX-XXXX).
        if ($reference !== '' && ($document = \App\Models\Document::where('verification_code', \App\Application\Documents\Security\VerificationCredentials::normalize($reference))->first())) {
    ...
    {
        // Canonical verification (result codes, integrity re-check, disclosure by confidentiality class):
        // App\Application\Documents\Security\DocumentVerificationPresenter.
        ...
        $eval = $presenter->evaluate($d, $policy, ['token' => $token]);
        DB::table('public_verification_lookups')->insert([
            'id' => (string) Str::uuid(), 'channel' => $channel, 'reference_hash' => hash('sha256', mb_strtoupper($reference)),
