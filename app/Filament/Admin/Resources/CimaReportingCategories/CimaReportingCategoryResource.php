...
namespace App\Filament\Admin\Resources\CimaReportingCategories;
...
final class CimaReportingCategoryResource extends Resource
{
    ...
    protected static ?string $modelLabel = 'reporting category';

    public static function table(Table $table): Table
    {
        return $table->defaultSort('sequence')->columns([
                Tables\Columns\TextColumn::make('kind')->badge()->formatStateUsing(fn (string $state) => $state === 'ART_411_CATEGORY' ? 'Art. 411 category' : 'Art. 557 intermediary measure'),
                Tables\Columns\TextColumn::make('sequence')->label('#')->sortable(),
                Tables\Columns\TextColumn::make('code')->searchable()->fontFamily('mono'),
                Tables\Columns\TextColumn::make('label_fr')->label('Libellé (FR)')->placeholder('-'),
