...
- Document signing key (generated 2026-09-26, id opesinsure-doc-2026-09) lives only on the server: /srv/opesinsure/shared/keys/document-signing-ed25519.key (0600). Keep an offline copy in your secret vault. If the key rotates, add the old public key to DOCUMENT_SIGNING_PUBLIC_KEYS so previously issued documents still verify.
