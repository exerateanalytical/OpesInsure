...
final class QuoteLifecycleController
{
 ...
 public function rate(Request $r,string $quote,QuoteService $s):JsonResponse{$q=$s->rate($this->quote($quote),$r->user());return response()->json(['data'=>['quote'=>$q,'offers'=>$q->offers()->orderBy('comparison_rank')->get()]]);}
