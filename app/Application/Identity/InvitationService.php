...
final class InvitationService
{
    ...
    public function issue(Tenant $tenant, User $actor, ?string $email, ?string $phone, string $roleCode, int $ttlHours = 72): array
    {
        ...
        $invitation = TenantInvitation::create([
            ...
            'role_code'=>$roleCode, 'token_hash'=>hash('sha256', $token), 'status'=>'PENDING', 'expires_at'=>now()->addHours($ttlHours), 'invited_by'=>$actor->id,
        ]);
    ...
    public function accept(string $token, User $user): TenantMembership
    {
        return DB::transaction(function () use ($token, $user): TenantMembership {
            ...
            $matches = ($invitation->recipient_email && hash_equals($invitation->recipient_email, mb_strtolower((string)$user->email)))
                || ($invitation->recipient_phone_e164 && hash_equals($invitation->recipient_phone_e164, $user->phone_e164));
            ...
            $this->audit->record('identity.invitation.accepted', 'tenant_invitation', $invitation->id, ['membership_id'=>$membership->id]);
            return $membership;
        });
    }
