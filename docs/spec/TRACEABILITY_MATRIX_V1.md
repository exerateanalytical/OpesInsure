...
| REQ-PRP-001 | Proposal state machine draft→submitted→reviewing→information_required→resubmitted→approved/declined; immutable submitted snapshot | BP II Proposal; WRS WF-015/016; CIMA Art.6 | DONE (6D) | ProposalMachine on StateMachineEngine (+INFORMATION_REQUIRED, RESUBMITTED; blueprintState()), proposal_submissions (append-only, hashed), proposal_status_history + workflow_transition_history | UnderwritingService (7B) to call ProposalService::applyUnderwritingDecision | REQ-WFL-001 | app/Application/Underwriting/ProposalService, ProposalMachine |
| REQ-PRP-002 | Declarations, consent, beneficiaries and market-conduct evidence (disclosure hash) | PRE §35; ICE gap 3; WRS WF-016 | PARTIAL (6D) | PROPOSAL questions via QuestionSetCatalogue (legacy disclosure adapter kept), proposal_declarations (append-only, text/answers/questions/terms hashes, channel, IP), KycGate + RuleEngine::assertComplete(BIND) at submit | Beneficiaries/suitability artefacts not linked; declaration texts legal_status UNVERIFIED | REQ-CRM-004 | app/Application/Underwriting/Proposal |
| REQ-PRP-003 | Risk-dependent document requirements MISSING/UPLOADED/REVIEWING/ACCEPTED/REJECTED/EXPIRED | PRE §36; DRM | DONE (6D) | ProposalDocumentRequirements over DocumentCatalogueService::requirementsFor(product, PRE_CONTRACT); document_requirement_versions no longer read (rows kept) | — | REQ-DOC-005 | app/Application/Underwriting/Proposal/ProposalDocumentRequirements |
| REQ-PRP-004 | POLICY_ISSUABLE gate (approved + payment condition + documents + UW + approvals) | PRE §37 | PARTIAL (6D) | PolicyIssuabilityService (evaluate/assertIssuable), GET proposals/{p}/issuability | Wire assertIssuable into PolicyIssuanceService (7C/7D); authority approvals (7A) | REQ-PRP-001, REQ-POL-008 | app/Application/Policies/PolicyIssuabilityService |
| REQ-PRP-005 | Effective-date rules, durations, instalment plans (SINGLE…CUSTOM) | PRE §38–42 | PARTIAL (6D) | cover_term_rules, CoverTermsService (rules, durations, schedule, resolveStart), PUT proposals/{p}/cover-terms | Authoring UI; issuance to use resolveStart; obligations per instalment (9A) | REQ-PAY-006 | app/Application/Policies/CoverTermsService |
