...
final class PaymentController{
 ...
 public function store(Request$r,PaymentRequestService$s,TenantContext$c):JsonResponse{$d=$r->validate(['proposal_id'=>'required|uuid|exists:proposals,id','provider'=>'required|in:fake,maviance,campay,mtn_momo,orange_money,bank_transfer,card_sandbox','payer_phone_e164'=>['required','regex:/^\+[1-9]\d{7,14}$/'],'idempotency_key'=>'required|string|min:16|max:128']);$p=$this->own->apply(Proposal::where('tenant_id',$c->id()),$r->user())->findOrFail($d['proposal_id']);DemoPersonas::assertProviderAllowed($d['provider'],$r->user());$i=$s->create(Tenant::findOrFail($c->id()),$p,$d,$r->user());return response()->json(['data'=>$i],$i->wasRecentlyCreated?201:200);}
 ...
 public function approveRefund(Request$r,string$refund,\App\Application\Finance\Refunds\RefundEngine$s):JsonResponse{$row=Refund::where('tenant_id',app(TenantContext::class)->id())->findOrFail($refund);return response()->json(['data'=>$s->approve($row,$r->user())]);}
}
