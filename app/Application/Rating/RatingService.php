...
final class RatingService
{
    ...
    public function price(TariffVersion $tariff, array $facts, string $lineCode, ?string $tenantId, ReferenceInstant $at, ?CarbonImmutable $asOf = null, array $context = []): array
    {
        ...
        foreach ($tax as $t) {
            $resolved['tax_levy:'.$t['key']] = $t['source_id'];
        }
        foreach ($fees as $f) {
        ...
        return ['pricing' => $pricing, 'engine_result' => $engineResult, 'resolved_versions' => $resolved,
            'tables' => ['tax_levy_version_id' => $tax[0]['source_id'] ?? null, 'fee_schedule_version_id' => $fees[0]['source_id'] ?? null]];
    }
    ...
    public function rateQuote(Quote $quote, TariffVersion $tariff, ReferenceInstant $at): array
    {
        ...
        $inputHash = $this->json->hash(['facts' => $quote->risk_facts, 'rules_hash' => $tariff->rules_hash]);
        ...
        try {
            ...
            DB::table('rating_runs')->insert([...$row, ...$r['tables'], 'status' => 'SUCCEEDED', 'output_snapshot' => $this->json->encode($output), 'output_hash' => $this->json->hash($output),
